# Use Vaadin in the cloud for a full-stack Java app

For more details on the code in this repository, read the developerWorks tutorial [Use Vaadin in the cloud for a full-stack Java app](http://www.ibm.com/developerworks/library/j-use-vaadin-in-the-cloud-for-a-full-stack-Java-app-bluemix/index.html).

The app is built on top of the Vaadin Bluemix boilerplate code, also known as Vaadin JPA Crud.  For your convenience, the full README for Vaadin Bluemix boilerplate follows.

### Deploying the tutorial's WAR file from this project to Bluemix

Due to a recent update of the Bluemix buildpack for Java, you may have difficulty using the default configuration profile of the deployed Liberty Profile server. In order to deploy the WAR file, set the following environment variable to configure Liberty Profile. 

```
cf setenv <your app name>  JBP_CONFIG_LIBERTY "app_archive: { implicit_cdi: true }"
```

You should also edit `manifest.yml` to reflect your app and database names. 

*full README from Vaadin Bluemix boilerplate follows* 
